...
class FileCoverage {
  ...
  serialise() {
    ...
    const functions = Array.from(this.functions).sort(
      ([, { line: l1 }], [, { line: l2 }]) => l1 - l2
    );
    const branches = Array.from(this.branches).sort(([b1], [b2]) => b1 - b2);
    const lines = Array.from(this.lines).sort(([l1], [l2]) => l1 - l2);
      ...
      .call(
        ...
        branches.map(([block, branches]) =>
          branches
            ...
            .filter(b => b) // remove un-assigned array indices
            .join('\n')
        ),
